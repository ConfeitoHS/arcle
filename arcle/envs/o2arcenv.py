...
from ..typing import ActionType, StateType, Operation
...
class O2ARCv2Env(AbstractARCEnv):
    ...
    def create_state_space(self):
        ...
        new_space_dict = {
            ...
            "clip_dim": spaces.Box(
                low=np.array([0, 0]), high=np.array([self.H, self.W]), dtype=np.int8
            ),
            "object_states": spaces.Dict(
                {
                    "active": spaces.MultiBinary(1),
                    "object": spaces.Box(
                        0, self.colors, (self.H, self.W), dtype=np.int8
                    ),
                    "object_sel": spaces.Box(0, 1, (self.H, self.W), dtype=np.int8),
                    "object_dim": spaces.Box(
                        low=np.array([0, 0]),
                        high=np.array([self.H, self.W]),
                        dtype=np.int8,
                    ),
                    "object_pos": spaces.Box(
                        low=np.array([-128, -128]),
                        high=np.array([127, 127]),
                        dtype=np.int8,
                    ),
    ...
    def create_action_space(self, action_count) -> Any:
        return spaces.Dict(
            {
                "selection": spaces.Box(
                    0, 1, (self.H, self.W), dtype=np.int8
                ),  # selection Mask
    ...
    def reward(self, state=None) -> float:
        if state is None:
            return 0
    ...
    def render_ansi(self):
        ...
        print(
            f'Selected : {True if self.last_action is not None and  np.any(self.last_action["selection"]) else False}',
            end=" ",
        )
