...
class Loader(metaclass=ABCMeta):
    ...
    @abstractmethod
    def parse(self, **kwargs) -> List[TaskType]:
        """
    ...
    def pick(self, data_index: Optional[int] = None, **kwargs) -> TaskType:
        """
        Pick one problem from loaded data.
        """
        assert (
            self.data is not None and len(self.data) > 0
        ), "Dataset wasn't loaded properly"
...
class ARCLoader(Loader):
    ...
    def get_path(self, **kwargs):
        ...
        path = os.path.join(
            os.path.dirname(os.path.abspath(__file__)), "../arcs/ARC/data"
        )
        train = kwargs["train"]
        self.train = train
...
class MiniARCLoader(Loader):
    ...
    def get_path(self, **kwargs):
        ...
        path = os.path.join(
            os.path.dirname(os.path.abspath(__file__)), "../arcs/Mini-ARC/data/MiniARC"
        )
    ...
    def parse(self, **kwargs):
        ...
        for p in self._pathlist:
            with open(p) as fp:
                ...
                desc = {
                    "id": fns[-1].split(".")[-2],
                    "description": " ".join(fns[0:-1]).strip(),
                }
