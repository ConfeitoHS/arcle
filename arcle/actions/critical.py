...
def no_op(state: StateType, action: ActionType) -> None:
    pass


def reset_grid(state: StateType, action: ActionType) -> None:
    """
...
def copy_from_input(state: StateType, action: ActionType) -> None:
    """
...
def resize_grid(state: StateType, action: ActionType) -> None:
    """
...
def crop_grid(state: StateType, action: ActionType) -> None:
    """
    ...
    np.copyto(
        dst=patch,
        src=state["grid"][xmin : xmax + 1, ymin : ymax + 1],
        where=np.logical_and(
            action["selection"][xmin : xmax + 1, ymin : ymax + 1],
