...
def dfs(grid: GridType, grid_dim: DimType, point: Tuple[int, int]) -> NDArray:
        ...
        if not visit_mask[x, y]:
            ...
            for i in range(4):
                ...
                if (
                    (0 <= xnew < xmax)
                    and (0 <= ynew < ymax)
                    and (grid[xnew, ynew] == col)
                    and (not visit_mask[xnew, ynew])
                ):
                    _dfs(xnew, ynew)
                else:
                    continue
...
def dfs_area(grid: GridType, grid_dim: DimType, selection: GridType) -> NDArray:
...
def gen_color(color: int) -> Operation:
    """
    ...
    def colorf(state: StateType, action: ActionType) -> None:
...
def gen_flood_fill(color: int) -> Operation:
    """
    Generates FloodFill0 ~ FloodFill9 operations. Shortcut of `dfs` + `Color0`~`Color9`.
    ...
    def floodfillf(state: StateType, action: ActionType) -> None:
        sel = action["selection"]
        ...
        sel = dfs(state["grid"], state["grid_dim"], (int(x), int(y)))
        state["grid"] = ma.array(state["grid"], mask=sel).filled(fill_value=color)
