...
for idx, trace in enumerate(traces):
    ...
    obs, info = env.reset(
        options={
            "adaptation": False,
            "prob_index": findbyname(traces_info[idx][0]),
            "subprob_index": traces_info[idx][1],
        }
    )
    converted = []
    ...
    for i in range(len(converted)):
        ...
        h, w = obs["grid_dim"]
        if trace[i][3].shape != (h, w) or np.any(
            obs["grid"][:h, :w] != trace[i][3].astype(np.int8)
        ):
...
print(
    f"Tested: {tested}, Passed: {(tested-len(error_trace)-len(failure_trace))/(tested)*100:.2f}%"
)
